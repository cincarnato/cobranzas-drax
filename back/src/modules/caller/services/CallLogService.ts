...
import type {IDraxFieldFilter, IDraxPaginateOptions, IDraxPaginateResult} from "@drax/crud-share";

type RegisterAttemptPayload = Pick<ICallLogBase, 'notes' | 'typification' | 'state' | 'promiseDate' | 'done'>
type ExportCallLogResult = {
    buffer: Buffer
    fileName: string
}
...
class CallLogService extends AbstractService<ICallLog, ICallLogBase, ICallLogBase> {
    ...
    get callLogRepository(): ICallLogRepository {
        return this._repository as ICallLogRepository
    }
    ...
    async registerAttempt(id: string, payload: RegisterAttemptPayload, userId?: string): Promise<ICallLog> {
    ...
    private getStateCounterDeltas(previousState?: string, nextState?: string) {
        return {
    ...
    private stateCounterDelta(previousState: string | undefined, nextState: string | undefined, state: string) {
        return (nextState === state ? 1 : 0) - (previousState === state ? 1 : 0)
    }

    async exportExcel(callListId: string): Promise<ExportCallLogResult> {
        const callListService = CallListServiceFactory.instance
        const callList = await callListService.findById(callListId)
    ...
    private collectDataKeys(rows: ICallLog[]) {
    ...
    private extractCallListIdFromFilters(filters: IDraxFieldFilter[]) {
